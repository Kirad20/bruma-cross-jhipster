...
export class AlertErrorComponent implements OnDestroy {
  ...
  constructor() {
    this.errorListener = this.eventManager.subscribe('brumacrosstrainingApp.error', (response: EventWithContent<unknown> | string) => {
      const errorResponse = (response as EventWithContent<AlertError>).content;
      this.addErrorAlert(errorResponse.message, errorResponse.key, errorResponse.params);
    });

    this.httpErrorListener = this.eventManager.subscribe(
      'brumacrosstrainingApp.httpError',
  ...
  private handleFieldsError(httpErrorResponse: HttpErrorResponse): void {
    ...
    for (const fieldError of fieldErrors) {
      ...
      const fieldName: string = this.translateService.instant(`brumacrosstrainingApp.${fieldError.objectName as string}.${convertedField}`);
